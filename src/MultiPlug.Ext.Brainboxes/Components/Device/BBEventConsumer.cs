...
namespace MultiPlug.Ext.Brainboxes.Components.Device
{
    public class BBEventConsumer : EventConsumer
    {
        ...
        public override void OnEvent(Payload e)
        {
            ...
            {
                if(string.Equals(p.Type, m_Properties.HighSubject, StringComparison.OrdinalIgnoreCase) && string.Equals(p.Value, m_Properties.HighValue, StringComparison.OrdinalIgnoreCase))
                {
                    Found = true;
                    m_Properties.IOLine.Value = 1;
                    break;
                }
                if (string.Equals(p.Type, m_Properties.LowSubject, StringComparison.OrdinalIgnoreCase) && string.Equals(p.Value, m_Properties.LowValue, StringComparison.OrdinalIgnoreCase))
                {
                    Found = true;
